...
def CreateLog(FolderName,sender, app_password, reciever, subject, body):
    ...
    for info in Data:
        ...
        fobj.write("Memory %% : %.2f\n \n" %info.get("memory_percent"))
         ...
         # New open file feature
        open_files_list = info.get("open_files", [])
        fobj.write("Files open : %d\n" % len(open_files_list))    
        fobj.write(Border+"\n")

        # Memory allocation
        rss_mb = info.get("rss", 0) / (1024 * 1024)
        vms_mb = info.get("vms", 0) / (1024 * 1024)
        fobj.write("Actual Memory (RSS) : %.2f MB\n" % rss_mb)
    ...
    Q4Part1.send_mail(sender, app_password, reciever, subject, body, FileName)
...
"""
    ...
    #python3 Demo.py 5 Marvellous
        ...
        schedule.every(int(sys.argv[1])).minutes.do(CreateLog,sys.argv[2],sender_mail, app_password, reciever_email, subject, body, )
