...
def ExtractBackup(zip_file,Destination):
...
# History of BACKUP:
def UpdateHistory(files_count, zip_file):
    ...
    zip_size = round(zip_size,2)

    with open(history_file, "a") as fobj:
        fobj.write(f"{date} | Files: {files_count} | Zip Size: {zip_size} KB\n")
...
def MarvellousDataShieldStart(sender, app_password, reciever, subject, body,Source="Data"):
    ...
    log_file, log_filename = CreateLog("LogFile_Q5")
...
"""
    ...
    # Restore the Files at desired destination
        ...
        ExtractBackup(zip_file, destination)
