...
def LoadingDataset(DataPath):
    ...
    # Manually Calculation the accuracy:
    print("Manually Calculation the accuracy")

    if len(Y_test) != len(Y_pred):
        raise ValueError("The Length of Y_test and Y_pred must be same")
    
    correct_predictions = 0
    for test_label, predicted_label in zip (Y_test,Y_pred):
    ...
    results = new_student.copy()
    results["PredictedResult"] = predictions
